...
import { MonitoringVariable, logger } from 'dart-api';
import {
  MenuItem,
  Select,
  SelectChangeEvent,
...
export default function SetGlobalValue(props: setReturnValueProps) {
  const { visible, variableList, selectedVarName, ChangeGlobalValue } = props;
  ...
  useEffect(() => {
    ...
    const variable = variableList.find(
      (item: MonitoringVariable) => item.name === selectedVarName,
    );
  ...
  const handleChangeGlobalValue = (e: SelectChangeEvent) => {
    ...
    const variable = variableList.find(
      (item: MonitoringVariable) => item.name === name,
    );
  ...
  return (
    <>
      ...
      >
        <FormControl sx={visible ? { display: 'flex' } : { display: 'none' }}>
          <FormGroup
            sx={{
              'width': '100%',
            }}
          >
            ...
            >
              {variableList &&
                variableList?.map(
                  (globalvalue: MonitoringVariable, index: number) => (
